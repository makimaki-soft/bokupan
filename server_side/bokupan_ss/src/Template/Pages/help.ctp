...
<body class="help">
    ...
    <div id="content">
        ...
        <section>
            ...
            <p>
            あなたはパンツをこよなく愛する紳士です。家宅にしｎ...訪問しパンツを得る、日々を過ごしていました。
            そんな折、パンツへの愛を競い合うコンテストの存在を耳にします。あなたは自身の愛が一番だと示すため、コンテストに参加することにしました。
            </p>
        </section>
        <section>
            <h4>画面の見方</h4>
            <?= $this->Html->image('', ['alt' => 'ゲーム画面']); ?>
            <p>
            マップ これが今回のコンテスト会場です（マップ全体の絵）
            ターゲットとなる家は９つあります。
            </p>
            <p>
            <?= $this->Html->image('', ['alt' => 'プレイヤー']); ?>
            <span>プレイヤー</span>
            </p>
            <p>
            <?= $this->Html->image('', ['alt' => '住人のいない家']); ?>
            <span>家です。</span>
            </p>
            <p>
            <?= $this->Html->image('', ['alt' => '住人のいる家']); ?>
            <span>家に住人がいる場合、住人アイコンが表示されます。</span>
            </p>
            <p>
            <?= $this->Html->image('', ['alt' => '自分の拠点']); ?>
            <span>自分の拠点です。拠点に変えるまでがパンツコンテンストです。</span>
            </p>
            <p>
            <?= $this->Html->image('', ['alt' => 'ライバルの拠点']); ?>
            <span>ライバルの拠点です。</span>
            </p>
            <p>
            <?= $this->Html->image('', ['alt' => '警察官']); ?>
            <span>巡回中の警察官です。見つかると・・・。</span>
            </p>
            <p>
            <?= $this->Html->image('', ['alt' => '巡回ルートの矢印']); ?>
            <span>巡回ルートを示す看板（矢印）がです。警察官は矢印に従って巡回します。</span>
            </p>

            <p>ステータス画面</p>
            <?= $this->Html->image('', ['alt' => '自分のステータス']); ?>

            <p>① ぽっけにいれているパンツ</p>
            <p>② 拠点に持ち帰ったパンツ</p>
            <p>③ コンテスト主催者から配られたアイテム。紳士に奇跡を起こします。</p>
        ...
        <select>
            <h4>コンテストの終了について</h4>
            すべての家のパンツを最初にコレクションに加えたプレイヤーの勝利となり、コンテンストの修了となります。
        </select>
    </div>
    <footer>
